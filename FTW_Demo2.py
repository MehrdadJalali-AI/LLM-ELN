...
def generate_response(question):
    # Initialize generated_response before the loop
    generated_response = ""
    ...
    for filename in os.listdir(pdfs_directory):
        if filename.endswith(".pdf"):
            ...
            # Assign the generated response to the variable
            generated_response = response.choices[0].text.strip()
            
            responses.append(f"Generated Response for {filename}:")
                        # Add a line of asterisks after each response
            formatted_response = generated_response + "\n" 
            responses.append(formatted_response)
            responses.append("*" * 50)
            # Add a line of asterisks with the filename label
            
    
    # Return the list of formatted responses
    return "\n".join(responses)
